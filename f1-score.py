...
def mytest(image_path, mask_path, conf_thres, iou_thres, expand, topk, fast):
    '''
    ...
    with torch.no_grad():
        ...
        for i, img_name in enumerate(os.listdir(image_path)):
            ...
            # output, mask_maps, _, _ = model(input.to(device), max_det_num=0, conf_thres=conf_thres, iou_thres=iou_thres,
            #                                 expand=expand, topk=topk, fast=fast)
            #
            ...
            probability_map = input_img
            accuracy,prec, recall, F1 = calculateF1Measure(probability_map, target, 0.5)
            average_accuracy=(average_accuracy*i+accuracy)/(i+1)
            average_F1 = (average_F1 * i + F1) / (i + 1)
            average_prec = (average_prec * i + prec) / (i + 1)
            average_recall = (average_recall * i + recall) / (i + 1)
    print(f'acc:{average_accuracy} prec:{average_prec}  recall:{average_recall}  F1: {average_F1} ')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--test_image", type=str, default="./data/test_CAVE/stuffed_toys_ms/AE/", help="path to load testing image")
    parser.add_argument("--test_mask", type=str, default="./data/test_CAVE/stuffed_toys_ms/ground-truth/", help="path to load testing masks")
    ...
    parser.add_argument("--expand", type=int, default=8,
                        help="The additonal side length of expanded local region for semantic generator")
    ...
    mytest(args.test_image, args.test_mask,  args.conf_thres, args.iou_thres, args.expand, args.topk,
         args.fast)
