...
from model.model_sunet.models import FFDNet
...
model = FFDNet(num_input_channels=1,opt=opt).to(device)
model.apply(weights_init_kaiming)

model = nn.DataParallel(model, device_ids=[0]).cuda()
model.load_state_dict(torch.load('pretrained_models/net_SUNet.pth'))
...
def run():
    ...
    for t, ms in enumerate(test_loader):
        ...
        while i + block_size <= max_rol:
            ...
            while j + block_size <= max_col:
                ...
                for t in tqdm(range(30)):
                    ...
                    net_input=x.view(block_size, block_size)
        ...
        cv2.imwrite("test_out/orig10.bmp",ms)
        Rec_ffdnet = Rec_ffdnet * 255.0
        cv2.imwrite("test_out/Rec_SUNet_10.bmp",Rec_ffdnet)
